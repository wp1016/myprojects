...
} elseif ( $dopost=='success' )
{
    ...
    if ( empty($site_id) )
    {
        ShowMsg("当前系统尚无绑定站点ID，请在系统后台网匠站内搜索模块中绑定！","javascript:;");
        exit();
    }
    $msg = <<<EOT
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#DADADA">
	<tbody>
		...
		<tr>
			<td height="30" colspan="2" style="color:#999"><strong>网匠站内搜索</strong>网匠站内搜索旨在帮助站长低成本地为网站用户提供高质量的网站内搜索服务。使用网匠站内搜索工具，您可以轻松打造网站专属的搜索引擎，自定义个性化的展现样式、功能模块等，并通过搜索广告获得收入。</td>
		</tr>
		</tbody>
		</table>
		</td>
...
当您看到这个页面的时候，则说明已经完成DedeCMS网匠站内搜索模块的安装和站点绑定。<br />
您还需要完成以下步骤才能完成DedeCMS网匠站内搜索功能的部署：<br/><br/>
1.<b>提交索引</b>：登录系统后台，在【模块】-【网匠站内搜索】-【站内搜索】中提交站点索引，如果您是第一次使用，需要选择【重新提交全量索引】，如果您已经提交过，每次添加完新内容后则直接使用【提交增量索引】功能；<br/><br/>
2.<b>部署代码</b>：您在模板对应位置插入标签 <font color='red'>{dede:baidusitemap/}</font>，或者可以直接复制下面文本框中的代码，粘贴到模板对应位置，重新更新HTML，即可查看网匠搜索框。<br/><br/>
...
<script type="text/javascript">document.write(unescape('%3Cdiv id="bdcs"%3E%3C/div%3E%3Cscript charset="utf-8" src="http://znsv.baidu.com/customer_search/api/js?sid={$site_id}') + '&plate_url=' + (encodeURIComponent(window.location.href)) + '&t=' + (Math.ceil(new Date()/3600000)) + unescape('"%3E%3C/script%3E'));</script>
也可以登录后台，在【模块】-【网匠站内搜索】-【搜索框管理】中根据自己需求创建符合站点样式的搜索框。<br/><br/>
CopyRight 2010  DesDev Inc. All rights reserved Powered by <a target="_blank" href="http://www.dedecms.com/">DedeCMS</a> <a target="_blank" href="网匠站内搜索模块">网匠站内搜索模块</a>
</div>
EOT;
    $msg = "<div style=\"line-height:36px;\">{$msg}</div>";

    $wintitle = '网匠站内搜索';
    $wecome_info = 'DedeCMS网匠站内搜索模块 》';
    $win = new OxWindow();
