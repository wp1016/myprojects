    ...
    if ( empty($sign) )
    {
        ...
        if($data['status']==0){
            ...
            if($output['status']==0){
                ...
                ShowMsg('成功同网匠站点API完成通信，下面进行索引提交……','?dopost=auth&sign='.$sign.'&action='.$action);
            } else {
                ShowMsg("提交网匠索引失败，无法校验本地密钥！远程接口服务器无法正常获取到您的站点文件！ <a href='http://www.dedecms.com/addons/baidusitemap/#help' target='_blank'>点击获取更多帮助</a>","javascript:;");
                exit();
            }
        }
    } else {
        ...
        {
            ShowMsg("网匠站内索引分析完成，进入提交页完成索引提交……","?dopost=submit&type=".$type);
            exit();
        } else {
            ShowMsg("提交网匠索引失败","?");
            exit();
		...
		<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#DFF9AA" height="100%">
			<tr>
				<td height="28" style="border:1px solid #DADADA" background='images/wbg.gif'>
					<div style="float:left">&nbsp;<b>◇<a href="?">◇网匠站内搜索 》结构化数据提交::索引管理</b>
					</div>
...
function isGoUrl(url,msg)
{
    ...
    $wintitle = '索引列表管理';
    $wecome_info = '<a href=\'baidusitemap_main.php\'>网匠站内搜索</a> 》结构化数据提交::索引管理';
    $win = new OxWindow();
			...
			<td colspan="2" height="100">
				<table width="98%" border="0" cellspacing="1" cellpadding="1">
					<tbody>
						...
						<tr>
							<td width="16%" height="30">网匠接口地址：</td>
